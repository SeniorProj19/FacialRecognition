...
@app.route('/match')
#def matching(userA_id, userB_picture):
def matching():
    cur = mydb.cursor(dictionary=True, buffered=True)
    cur.execute("SELECT * FROM photos")
    result = cur.fetchall() #json file of all photos with userids
...
def searchFaceHelper(userA_id, userB_picturename):
    ...
    #changed
    #image_file_name = userB_picturename)
...
def searchFace():
    ...
    for fileNames in face_file_names:
        fcol.add_face(COLLECT_NAME, fileNames)
    #pprint(fcol.list_faces(COLLECT_NAME))
    image_file_name = str(Path(FACE_SEARCH_DIR) /  'search.jpg')
    faces_info = fcol.find_face(COLLECT_NAME, image_file_name)
    print('Found', len(faces_info), 'match' + ('' if len(faces_info) == 1 else 's'))
    # Extract the name of the reference image(s) that were matched
    #pprint([face_info['Face']['ExternalImageId'] for face_info in faces_info])
    matchingFileName = [face_info['Face']['ExternalImageId'] for face_info in faces_info]
...
def text():
    ...
    #matchingFileName = [face_info['Face']['ExternalImageId'] for face_info in faces_info]
    #matchingFileName = str(matchingFileName)
    matchingFileName = '[u\'elonmusk.jpg\']'
    print(matchingFileName)
    m = re.search(r"\'[^']+\'",matchingFileName)
    if m:
        print(m.group(0))
    else:
...
def getConnections(user_id):
     cur = mydb.cursor(dictionary=True)
     statement = "SELECT * FROM information WHERE user_id IN (SELECT paired_user FROM relations WHERE user_id = "+str(user_id)+")"
     cur.execute(statement)
     result = cur.fetchall()
     for ppl in result:
        ppl.update(birthday = str(ppl['birthday']))
